                ...
                for simulation in SimInfo:
                    ...
                    if simulation in all_sim_masses:
                        if len(all_sim_masses[simulation]) == len(SimInfo[simulation]['goodhalos']):
                            ...
                            if all('Mb/Mtot_within_reff' in all_sim_masses[simulation][str(hid)] for hid in SimInfo[simulation]['goodhalos']):
                                if verbose:
                    ...
                    for hid in halos:
                        ...
                        try:
                            ...
                            try:
                                try:
                                    rvir, Mvir, Mstar, Mgas = ahf_halo.properties['Rhalo'], ahf_halo.properties['Mhalo'], ahf_halo.properties['M_star'], ahf_halo.properties['M_gas']
                                except:
                                    rvir, Mvir, Mstar, Mgas = ahf_halo.properties['Rvir'], ahf_halo.properties['mass'], ahf_halo.properties['M_star'], ahf_halo.properties['M_gas']

                                rvir, Mvir, Mstar, Mgas = pynbody.array.SimArray(rvir, 'kpc'), pynbody.array.SimArray(Mvir, 'Msol'), pynbody.array.SimArray(Mstar, 'Msol'), pynbody.array.SimArray(Mgas, 'Msol')
                            ...
                            try:
                                Mvir_within_reff, Mstar_within_reff, Mgas_within_reff, mb_mvir_within_reff,HI_within_reff = mass_properties_within_r(halo, Reff)
                            except:
                                print(f"Failed to calculate mass within Reff for halo {hid} in simulation {simulation}.")
                                print(traceback.format_exc())
                            ...
                            print(f'Baryon fraction within Reff: {mb_mvir_within_reff} for halo {hid} in simulation {simulation}.')
