...
from Functions import Ellipsoid,Project,kpc2pix,pix2kpc,Project_OLD
...
# Load SimulationInfo pickle file
...
# Extract halos information
halos = SimInfo[args.simulation]['goodhalos']

# Load Images pickle file
images_file_path = data_dir / f'{args.simulation}.{args.feedback}.Images.pickle'
with open(images_file_path, 'rb') as file:
    Images = pickle.load(file)

# Load Profiles pickle file
profiles_file_path = data_dir / f'{args.simulation}.{args.feedback}.Profiles.pickle'
with open(profiles_file_path, 'rb') as file:
    Profiles = pickle.load(file)

# Load 3DShapes pickle file
shapes_file_path = data_dir / f'{args.simulation}.{args.feedback}.3DShapes.pickle'
with open(shapes_file_path, 'rb') as file:
        ...
        for xrot in np.arange(0,180,30):
            for yrot in np.arange(0,360,30):
                ...
                try:
                    ...
                    # Initially assign a, ba, ca, Es from the same index
                    a, ba, ca, Es = [Shapes[str(hid)]['a'][ind_eff], Shapes[str(hid)]['ba'][ind_eff],
                                     Shapes[str(hid)]['ca'][ind_eff], Shapes[str(hid)]['Es'][ind_eff]]
                    
                    # Check if 'a' is 0 and adjust accordingly
                    if a == 0:
                        # Define indices for previous and next bins, ensuring they are within valid range
                        prev_index = max(ind_eff - 1, 0)
                        next_index = min(ind_eff + 1, len(Shapes[str(hid)]['a']) - 1)
                        
                        prev_a = Shapes[str(hid)]['a'][prev_index]
                        next_a = Shapes[str(hid)]['a'][next_index]
                        
                        # Determine which indices to use based on the conditions
                        if prev_a != 0 and next_a != 0:
                            # Use np.abs() for element-wise absolute value calculation on a list
                            selected_index = (prev_index, next_index)[np.argmin(np.abs([prev_a - Rhalf, next_a - Rhalf]))]
                        elif prev_a != 0:
                            selected_index = prev_index
                        elif next_a != 0:
                            selected_index = next_index
                        else:
                            selected_index = ind_eff  # Fallback to the original index if both are 0
                            
                        
                        # Update a, ba, ca, Es based on the selected index
                        a, ba, ca, Es = [Shapes[str(hid)]['a'][selected_index], Shapes[str(hid)]['ba'][selected_index],
                                         Shapes[str(hid)]['ca'][selected_index], Shapes[str(hid)]['Es'][selected_index]]
                    if 'ba_smooth' in Shapes[str(hid)]:
                    ...
                    if xrot ==0 and yrot ==0:
                        print(f"An error occurred during projection halo {hid} calculations: {e}")
                        #traceback.print_exc()
                ...
                if args.plot:
                    ...
                    Image_File = root_dir / f'Figures/Images/{args.simulation}.{args.feedback}/{hid}/{hid}.x{xrot:03d}.y{yrot:03d}.png'
                    LogImage = plt.imread(Image_File)
                    ax.imshow(LogImage)
                    ax.set_xlim([0,1e3])
                    ax.set_ylim([1e3,0])
                    ...
                    Fig_path = root_dir / f'Figures/Images/{args.simulation}.{args.feedback}/{hid}/{hid}.x{xrot:03d}.y{yrot:03d}.Intrinsic.png'  
...
file_path = data_dir / f'{args.simulation}.{args.feedback}.ProjectedData.pickle'
