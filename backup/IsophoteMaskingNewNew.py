...
def updateChart(iso, _VARS,halo, rotation, DrawCircle=False, DrawAngle=False, DrawEllipse=False, DrawUserEllipse=False):
...
def load_and_display_current_image(Images, Profiles, halos, current_halo_index, current_rotation_index, sim, feedback, _VARS):
    ...
    if rotation:  # Check if rotation is valid (not None)
        ...
        updateChart(iso,_VARS, halo, rotation, DrawCircle=False, DrawEllipse=False, DrawUserEllipse=False)  # Adapt parameters as needed


#define function to get the current halo, and rotation and intialize the first halo, and rotation
def initalize_simulation(feedback,sim):
    Images = pickle.load(open(data_dir / f'{sim}.{feedback}.Images.pickle', 'rb'))
    print(Images.keys())
    ShapeData = pickle.load(open(data_dir / f'{sim}.{feedback}.ShapeData.pickle', 'rb'))
    Masking = pickle.load(open(data_dir / f'{sim}.{feedback}.Masking.pickle', 'rb'))
    Profiles = pickle.load(open(data_dir / f'{sim}.{feedback}.Profiles.pickle', 'rb'))
    ...
    load_and_display_current_image(Images, Profiles, halos, current_halo_index, current_rotation_index, sim, feedback, _VARS)
    new_view = False
    return Images,Image,ShapeData,Masking,Profiles,_VARS
# what i want 
...
def filter_dictionary(Masking):
    filtered_sims = {}
    for sim, halos in Masking.items():
        filtered_halos = {}
        for halo, rotations in halos.items():
            filtered_rotations = {rotation: value for rotation, value in rotations.items() if value is not False}
            if filtered_rotations:
                filtered_halos[halo] = filtered_rotations
        if filtered_halos:
            filtered_sims[sim] = filtered_halos
    return filtered_sims

def update_other_dictionaries(filtered_dict, *other_dicts):
    for other_dict in other_dicts:
        for sim in list(other_dict.keys()):
            if sim not in filtered_dict:
                del other_dict[sim]
            else:
                for halo in list(other_dict[sim].keys()):
                    if halo not in filtered_dict[sim]:
                        del other_dict[sim][halo]
    return other_dicts
...
def load_data(feedback ='RDZ', redo_masking = False):
    feedback = 'RDZ'
    #create a new dictoinary to hold all simulations info and feedbacks
    ...
    try:
        ...
        #load the pickle file using pathlib
        else:
            print(f'Simulation Info not found for {feedback} feedback model')
            sys.exit()

        #add the sims to the SimulationInfo dictionary
        SimulationInfo = sim_info
        Masking = {}
        Profiles = {}
        ShapeData = {}

        #load existing store in dictornalry, with entries for each feedback, subentries for each sim, and subsubentries
        for sim in sim_info:
            sim_mask = pickle.load(open(data_dir / f'{sim}.{feedback}.Masking.pickle', 'rb'))
            Masking[sim] = sim_mask
            Profiles[sim] = pickle.load(open(data_dir / f'{sim}.{feedback}.Profiles.pickle', 'rb'))
            ShapeData[sim] = pickle.load(open(data_dir / f'{sim}.{feedback}.ShapeData.pickle', 'rb'))
    ...
    if not redo_masking:
        Masking = filter_dictionary(Masking)
        SimulationInfo, Profiles, ShapeData = update_other_dictionaries(Masking, SimulationInfo, Profiles, ShapeData)

    return SimulationInfo, Masking, Profiles, ShapeData
...
def main():
    ...
    SimulationInfo, Masking, Profiles, ShapeData = load_data(redo_masking=redo_masking)
    ...
    while main_loop:  # Main event loop
        ...
        #defie current working feedback
        feedback = 'RDZ'
        ...
        if new_sim:
            Images, Image, ShapeData, Masking, Profiles, _VARS = initalize_simulation(feedback, sim)
            new_sim = False
            load_and_display_current_image(Images, Profiles, halos, current_halo_index, current_rotation_index, sim,
                                           feedback, _VARS)
        halo, rotation = get_current_halo_rotation(Images, halos, current_halo_index, current_rotation_index)
        ...
        if event in [sg.WIN_CLOSED, 'Exit']:
            ...
            sys.exit()
        if event in ['Next Simulation', 'Previous Simulation', 'Next Feedback', 'Previous Feedback']:
            if event == 'Next Simulation':
                ...
                current_halo_index = 0
                current_rotation_index = 0
            elif event == 'Next Feedback':
                current_feedback_index = min(current_feedback_index + 1, len(feedbacks) - 1)
                current_sim_index = 0
                current_halo_index = 0
                current_rotation_index = 0
            elif event == 'Previous Feedback':
                current_feedback_index = max(current_feedback_index - 1, 0)
                current_sim_index = 0
                current_halo_index = 0
                current_rotation_index = 0
            new_sim = True
        if event in ['Next Halo', 'Previous Halo', 'Next Rotation', 'Previous Rotation']:
            ...
            if event == 'Next Halo':
                ...
                rotations = list(Images[halo[current_halo_index]].keys())
        ...
        if event in ['Save', 'Ignore']:
            ...
            pickle.dump(ShapeData, open(f'../../Data/{sim}.{feedback}.ShapeData.pickle', 'wb'))
            Masking[halo][rotation] = False
            pickle.dump(Masking, open(f'../../Data/{sim}.{feedback}.Masking.pickle', 'wb'))
        ...
        if event == 'Reset':
            ...
            DC, DE = False, False  # Updated: Removed DA
            updateChart(iso, _VARS, halo, rotation,DrawCircle=DC, DrawEllipse=DE)
        if event == 'Mask': #keep track of iso, and masked iso seperately
            ...
            if DC:
                ...
                updateChart(iso, _VARS,halo, rotation, DrawEllipse=DE)
        ...
        if event == 'Auto Mask':
            ...
            updateChart(iso, _VARS,halo, rotation, DrawCircle=DC, DrawEllipse=True)
        ...
        if event == 'Draw Ellipse':
            if values['EllipseMode']:  # Check if the list is not empty
                ...
                updateChart(iso, _VARS,halo, rotation,  DrawUserEllipse=True)
            else:
